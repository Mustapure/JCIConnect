...
$businesses = [];
$countStmt = $db->query("SELECT COUNT(*) FROM businesses WHERE status = 'active'");
...
$cities = [];
$cityCountStmt = $db->query("SELECT COUNT(DISTINCT city) FROM businesses WHERE status = 'active'");
if ($cityCountStmt->fetchColumn() > 0) {
    $cityResult = $db->query("SELECT DISTINCT city FROM businesses WHERE status = 'active' ORDER BY city");
    while ($row = $cityResult->fetch(PDO::FETCH_ASSOC)) {
        $cities[] = $row['city'];
    }
}
?>
...
$location = isset($biz['website']) && $biz['website'] ? $biz['website'] : (isset($biz['address']) ? $biz['address'] : $biz['city']);
