...
// Use shared constants/config from database.php (already defined there)
$message = '';
$messageType = '';

define('CSRF_SECRET', 'jci_zone12_secret_key_2024');

// Shared functions (reuse from task best practices)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (empty($errors)) {
            // Insert using PDO (contact_submissions table)
            $db = getDB();
            $stmt = $db->prepare("INSERT INTO contact_submissions (name, email, phone, city, message) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$name, $email, $phone, $city, $messageText]);
